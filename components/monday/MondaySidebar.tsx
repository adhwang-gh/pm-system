      ...
      <div style={{ width: 44, display: 'flex', flexDirection: 'column', alignItems: 'center', background: BG, borderRight: `1px solid ${BORDER}`, paddingTop: 12, gap: 6 }}>
        ...
        {NAV.map(n => (
          <button key={n.label} onClick={() => onNavSelect(n.view)} title={n.label}
            style={{ width: 32, height: 32, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, cursor: 'pointer', border: 'none', background: activeNav === n.view ? `${GOLD}18` : 'transparent', color: activeNav === n.view ? GOLD : MUTED, transition: 'all 0.15s' }}>
            {n.icon}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ width: 220, display: 'flex', flexDirection: 'column', background: BG, borderRight: `1px solid ${BORDER}` }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '16px 16px 14px', borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ width: 22, height: 22, borderRadius: 3, background: GOLD, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, color: '#FFFFFF', flexShrink: 0, ...JOST }}>P</div>
      ...
      <div style={{ padding: '8px 8px', borderBottom: `1px solid ${BORDER}` }}>
        {NAV.map(n => (
          <button key={n.label} onClick={() => onNavSelect(n.view)}
            style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 10, fontSize: 12, cursor: 'pointer', border: 'none', textAlign: 'left', background: activeNav === n.view ? `${GOLD}15` : 'transparent', color: activeNav === n.view ? GOLD : MUTED, fontWeight: activeNav === n.view ? 500 : 400, transition: 'all 0.15s', ...JOST, letterSpacing: '0.02em' }}>
            ...
            {boards.map(board => {
              ...
              return (
                <button key={board.id} onClick={() => { onNavSelect('board'); onSelectBoard(board.id) }}
                  style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 10, fontSize: 12, cursor: 'pointer', border: 'none', textAlign: 'left', background: active ? `${GOLD}15` : 'transparent', color: active ? GOLD : MUTED, borderLeft: active ? `2px solid ${GOLD}` : '2px solid transparent', transition: 'all 0.15s', ...JOST }}>
            ...
            {showNewBoard ? (
              <div style={{ padding: '6px 4px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <input autoFocus value={newBoardTitle} onChange={e => setNewBoardTitle(e.target.value)}
                  ...
                  style={{ background: SURFACE, border: `1px solid ${GOLD}44`, borderRadius: 10, padding: '6px 10px', fontSize: 12, color: TEXT, outline: 'none', width: '100%', boxSizing: 'border-box', ...JOST }} />
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={confirmNewBoard} disabled={!newBoardTitle.trim()}
                    style={{ flex: 1, padding: '5px 0', background: GOLD, color: '#FFFFFF', border: 'none', borderRadius: 10, fontSize: 11, fontWeight: 600, cursor: 'pointer', opacity: newBoardTitle.trim() ? 1 : 0.4, ...JOST }}>Create</button>
                  <button onClick={() => { setShowNewBoard(false); setNewBoardTitle('') }}
                    style={{ flex: 1, padding: '5px 0', background: SURFACE, color: MUTED, border: `1px solid ${BORDER}`, borderRadius: 10, fontSize: 11, cursor: 'pointer' }}>Cancel</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setShowNewBoard(true)}
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 10, fontSize: 12, cursor: 'pointer', border: 'none', textAlign: 'left', background: 'transparent', color: FAINT, ...JOST }}>
                + Add workspace
              </button>
            )}
          </div>
